...
Sanitizer builds: `just asan` (Linux) or `just ubsan` (Windows). Pass extra Meson options through, e.g. `just setup-release -Dbundled_libsixel=disabled` (see [Build options](#build-options)).
