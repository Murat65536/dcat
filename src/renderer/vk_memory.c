...
bool create_buffer(VulkanRenderer* r, const VkDeviceSize size, const VkBufferUsageFlags usage,
    ...
    *buffer = VK_NULL_HANDLE;
    ...
    if (result != VK_SUCCESS) {
        vulkan_renderer_set_error(r, result, "vkCreateBuffer",
    ...
    if (!find_memory_type(r, mem_req.memoryTypeBits, properties,
    ...
    if (result != VK_SUCCESS) {
        vulkan_renderer_set_error(r, result, "vkBindBufferMemory",
    ...
    if (properties & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) {
        ...
        if (result != VK_SUCCESS) {
            vulkan_renderer_set_error(r, result, "vkMapMemory",
...
bool create_image(VulkanRenderer* r, const uint32_t width, const uint32_t height, const VkFormat format,
    ...
    *image = VK_NULL_HANDLE;
    ...
    if (result != VK_SUCCESS) {
        vulkan_renderer_set_error(r, result, "vkCreateImage",
    ...
    if (result != VK_SUCCESS) {
        vulkan_renderer_set_error(r, result, "vkBindImageMemory",
...
VkImageView create_image_view(VulkanRenderer* r, VkImage image, VkFormat format, VkImageAspectFlags aspect_flags) {
    ...
    if (result != VK_SUCCESS) {
        vulkan_renderer_set_error(r, result, "vkCreateImageView",
